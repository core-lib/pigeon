...
public class BeanContextServicesSupport extends BeanContextSupport implements BeanContextServices, Serializable {
	...
	protected class BCSSProxyServiceProvider implements BeanContextServiceProvider, BeanContextServiceRevokedListener {
		...
		Object getService(BeanContextServices bcs, Object requestor, Class<?> serviceClass, Object serviceSelector, BeanContextServiceRevokedListener listener) throws TooManyListenersException {
			return backBCS.getService(BeanContextServicesSupport.this.getBeanContextServicesPeer(), requestor, serviceClass, serviceSelector, new ServiceRevokedListenerDelegator(listener));
		}
	...
	private class ServiceRevokedListenerDelegator implements BeanContextServiceRevokedListener {
		...
		public void serviceRevoked(BeanContextServiceRevokedEvent event) {
			backListener.serviceRevoked(new BeanContextServiceRevokedEvent(BeanContextServicesSupport.this.getBeanContextServicesPeer(), event.getServiceClass(), event.isCurrentServiceInvalidNow()));
		}
	 ...
	 * <code>BCSSServiceProvider</code> object. All access to this object should
	 * be synchronized on itself.
	 */
	 ...
	 */
	protected transient ArrayList<Object> bcsListeners;
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.googlecode.openbeans.beancontext.BeanContextServices#
	 * addBeanContextServicesListener
	 * (com.googlecode.openbeans.beancontext.BeanContextServicesListener)
	 */
	...
	protected void bcsPreSerializationHook(ObjectOutputStream oos) throws IOException {
		...
		synchronized (services) {
			...
			for (Iterator<Entry<Class<?>, BeanContextServicesSupport.BCSSServiceProvider>> iter = services.entrySet().iterator(); iter.hasNext();) {
				Entry<Class<?>, BeanContextServicesSupport.BCSSServiceProvider> entry = iter.next();
	...
	public void initialize() {
		...
		serializable = 0;
		proxy = null;
		bcsListeners = new ArrayList<Object>();
	}

	/**
	 * Called after the parent context is updated. The implementation checks if
	 * the parent context is a <code>BeanContextServices</code>. If it is, then
	 * a <code>BCSSProxyServiceProvider</code> is created to delegate service
	...
	private void releaseServiceWithoutCheck(BeanContextChild child, BCSSChild bcssChild, Object requestor, Object service, boolean callRevokedListener) {
		...
		synchronized (child) {
			// scan record
			for (Iterator<ServiceRecord> iter = bcssChild.serviceRecords.iterator(); iter.hasNext();) {
	...
	private void notifyServiceRevokedToServiceUsers(Class<?> serviceClass, BeanContextServiceProvider serviceProvider, boolean revokeCurrentServicesNow, BCSSChild bcssChild) {
		...
		synchronized (bcssChild.child) {
			for (Iterator<ServiceRecord> it = bcssChild.serviceRecords.iterator(); it.hasNext();) {
