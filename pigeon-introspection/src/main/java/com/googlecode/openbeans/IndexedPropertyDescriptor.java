...
public class IndexedPropertyDescriptor extends PropertyDescriptor {
	...
	public boolean equals(Object obj) {
		...
		return (super.equals(other) && (indexedPropertyType == null ? other.indexedPropertyType == null : indexedPropertyType.equals(other.indexedPropertyType))
	...
	private void internalSetIndexedReadMethod(Method indexGetter) throws IntrospectionException {
		...
		if (indexGetter == null) {
			if (indexedSetter == null) {
				if (getPropertyType() != null) {
					// beans.5A=Indexed method is not compatible with non
					// indexed method
		...
		if (indexedReadType == Void.TYPE) {
			...
			// beans.5A=Indexed read method is not compatible with indexed write
			// method
			throw new IntrospectionException(Messages.getString("beans.5A")); //$NON-NLS-1$
		}
		...
		if (this.indexedGetter == null) {
			...
			if (indexedPropertyType != indexedReadType) {
				// beans.5A=Indexed read method is not compatible with indexed
				// write method
				throw new IntrospectionException(Messages.getString("beans.5A")); //$NON-NLS-1$
			}
		}
	...
	private void internalSetIndexedWriteMethod(Method indexSetter, boolean initialize) throws IntrospectionException {
		...
		if (indexSetter == null) {
			if (indexedGetter == null) {
				if (getPropertyType() != null) {
					// beans.5E=Indexed method is not compatible with non
					// indexed method
		...
		if (indexedSetterArgs[0] != Integer.TYPE) {
			// beans.60=Indexed write method must take an int as its first
			// argument
		...
		if (initialize && indexedGetter == null) {
			...
			if (indexedPropertyType != indexedWriteType) {
				// beans.61=Indexed write method is not compatible with indexed
				// read method
