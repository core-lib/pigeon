...
/**
 * 密钥长度 必须为 112 或 168 位,初始化向量长度8位
 * 
 * @author yangchangpei
 * 
 */
public class DESedeInvocationEncryptor extends DefaultInvocationEncryptor implements InvocationEncryptor {
	private static final String ALGORITHM = "DESede/CBC/PKCS7Padding";
	private static final Random RANDOM = new Random();

	public DESedeInvocationEncryptor() {
	...
	public OutputStream wrap(Key key, int keysize, OutputStream outputStream) throws Exception {
		return new DESedeInvocationOutputStream(key, keysize, outputStream);
	}

	public InputStream wrap(Key key, int keysize, InputStream inputStream) throws Exception {
		return new DESedeInvocationInputStream(key, keysize, inputStream);
	}
