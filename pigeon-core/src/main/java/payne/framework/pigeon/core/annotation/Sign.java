...
import payne.framework.pigeon.core.processing.InvocationSignProcedure;
import payne.framework.pigeon.core.signature.InvocationSigner;

/**
 * 数据签名,使用数据签名功能需要对框架进行和使用{@link Encrypt}一样的配置,可以通过实现{@link InvocationSigner}
 * 对框架的签名算法进行拓展<br/>
 * 加上该注解的服务,服务端会用非对称私钥对数据进行签名,拼接在数据的后面并且用分隔符{@link Sign#separator()}分隔.<br/>
 * 注意.这里配置的编码方式{@link Sign#dataEncoding()}
 * ,是对数据的编码,签名数据的编码采用大写的HEX编码方式.客户端收到数据时先读取到分隔符.解码便得到数据,<br/>
