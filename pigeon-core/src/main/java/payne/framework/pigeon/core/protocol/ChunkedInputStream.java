 ...
 *
 * <p>
 * Note that this class NEVER closes the underlying stream, even when close gets called. Instead, it will read until the "end" of its chunking on close, which allows for the seamless invocation of subsequent HTTP 1.1 calls, while not
...
public class ChunkedInputStream extends InputStream {
	 ...
	 * @param off
	 *            The offset into the byte array at which bytes will start to be placed.
	 * @param len
	...
	private static int getChunkSizeFromInputStream(final InputStream in) throws IOException {
		...
		String dataString = new String(data, 0, data.length, "US-ASCII");
